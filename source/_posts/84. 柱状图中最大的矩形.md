...
class Solution {
    public int largestRectangleArea(int[] heights) {
        ...
        int i = 0;
        while (i <= len) {
            if (mono_stack.isEmpty() || heightnew[mono_stack.peek()] < heightnew[i]) {
                mono_stack.push(i);
                i++;
            } else {
                int h = mono_stack.pop();
                maxarea = Math.max(maxarea, (mono_stack.isEmpty() ? i : i - mono_stack.peek() - 1) * heightnew[h]);
            }
        }
        return maxarea;
    }
